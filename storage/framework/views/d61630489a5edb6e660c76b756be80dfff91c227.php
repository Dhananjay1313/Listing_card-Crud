    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on("click","#edit",function(){
                ...
                success: function(response) {
                    // console.log(response);
                    // console.log(response.alldata[1].role_type);
                    // console.log(response.alldata[1]);
                    // console.log(response.alldata[0].employee_data);
                    $("#id").val(response.alldata[1].id);
                    $("#firstname").val(response.alldata[1].firstname);
                    $("#lastname").val(response.alldata[1].lastname);
                    $("#email").val(response.alldata[1].email);
                    $("#password").val(response.alldata[1].password);
                    $('input[type="radio"]').filter('[value=' + response.alldata[1].gender + ']').prop("checked", true);
                    var image = "<img src='<?php echo e(asset('image/')); ?>/" + response.alldata[1].image + "' width='100px' height='100px'>";
                    $("#img").html(image);
                    $("#role").val(response.alldata[1].role);
                    console.log(response.alldata[1].role);
                    if (response.alldata[1].role == "employee") {
                        $('#managerDropdown').show();
                            var role_type = response.alldata[0].employee_data;
                            console.log(role_type);
                            console.log(response.alldata[1].role_type);
                            var html ='<option value="" selected disabled>Select role_type </option>';
                            for (let i = 0; i < role_type.length; i++) {
                                if (response.alldata[1].role_type == role_type[i].id) {
                                    html += "<option value='" + role_type[i].id +"' selected >" +role_type[i].employee + "</option>";
                                } else {
                                    html += "<option value='" + role_type[i].id + "'>" +role_type[i].employee +"</option>";
                                }
                            }
                        $('#role_type').html(html);
                    }else if (response.alldata[1].role == "manager") {
                        $('#managerDropdown').show();
                            var role_type = response.alldata[0].manager_data;
                            console.log(role_type);
                            console.log(response.alldata[1].role_type);
                            var html ='<option value="" selected disabled>Select role_type </option>';
                            for (let i = 0; i < role_type.length; i++) {
                                if (response.alldata[1].role_type == role_type[i].id) {
                                    html += "<option value='" + role_type[i].id +"' selected >" +role_type[i].manager + "</option>";
                                } else {
                                    html += "<option value='" + role_type[i].id + "'>" +role_type[i].manager +"</option>";
                                }
                            }
                        $('#role_type').html(html);
                    }
                    $("#status").val(response.alldata[1].status);
                }
                });
            });
